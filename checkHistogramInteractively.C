...
AnalysisTree->Draw("nJet","(passPresel_Ele && nJet>=2 && nBJet==0 && nPho==1 )*evtWeight*prefireSF*PUweight*muEffWeight*eleEffWeight*1.0*1.0*1.0*1.0*btagWeight_1a","hist");
}
...
// for 2016: AnalysisTree->Draw("phoEta:phoPhi","(passPresel_Ele && nJet==2 && nBJet==0 && nPho==1 && !(((phoPhi>2.7 && phoPhi<3.1) && phoEta>1.2)))*evtWeight*PUweight*muEffWeight*eleEffWeight*1.0*1.0*btagWeight_1a","colz");
