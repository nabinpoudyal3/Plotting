...
if looseCRge2e0:  #CR1+CR2+CR3
	plotDirectory = "ZJets_syst_looseCRge2e0plots_%s/"%(selYear)
	regionText = "N_{j}#geq2, N_{b}=0"

if looseCRe2e0:  #CR1     
	plotDirectory = "ZJets_syst_looseCRe2e0plots_%s/"%(selYear)
	regionText = "N_{j}=2, N_{b}=0"

if looseCRe3e0:  #CR2
	plotDirectory = "ZJets_syst_looseCRe3e0plots_%s/"%(selYear)
	regionText = "N_{j}=3, N_{b}=0"

if looseCRge4e0:  #CR3
	plotDirectory = "ZJets_syst_looseCRge4e0plots_%s/"%(selYear)
	regionText = "N_{j}#geq4, N_{b}=0"

if looseCRe2e1:  #C4
	plotDirectory = "ZJets_syst_looseCRe2e1plots_%s/"%(selYear)
	regionText = "N_{j}=2, N_{b}=1"

if looseCRe3e1:  #CR5
	plotDirectory = "ZJets_syst_looseCRe3e1plots_%s/"%(selYear)
	regionText = "N_{j}=3, N_{b}=1"
	
if looseCRe2e2:  #CR6
	plotDirectory = "ZJets_syst_looseCRe2e2plots_%s/"%(selYear)
	regionText = "N_{j}=2, N_{b}=2"

if looseCRe3ge2:  #CR7
	plotDirectory = "ZJets_syst_looseCRe3ge2plots_%s/"%(selYear)
...
myDir = {"ele/myZJets":"ele_myZJets","ele/myBackground":"ele_myBackground",
          "mu/myZJets":"mu_myZJets", "mu/myBackground":"mu_myBackground"}
...
if not os.path.exists("upDownTemplateZJets/"+plotDirectory):
	os.mkdir("upDownTemplateZJets/"+plotDirectory)
	

filename = "ZJets_syst_Prefit.root"
systematicList = ["BTagSF_b","BTagSF_l","MuEff","EleEff","PhoEff","PU"]# ,"Q2","isr", "fsr", "prefireEcal", "JER", "JECTotal"] # keep adding more systematics 
...
for idir in myDir.keys():
	...
	for item in systematicList:
		h_nominal = myfile.Get(idir+"/nominal");h_nominal.SetTitle(";M_{l,l}(GeV);Events")
		...
		ratioUp.GetXaxis().SetTitle('m_{l,l} GeV %s'%item)
		...
		c.RedrawAxis()
		#c.Print("%s%s_%s.pdf"%(plotDirectory,myDir[idir],item))
		c.Print("upDownTemplateZJets/%s%s_%s.pdf"%(plotDirectory,myDir[idir],item))
		c.Close()
