...
for mysample in ${SampleList[@]}; do
	...
	python makeHistograms.py -c $channel -y $year --$controlRegion -s $mysample --syst $systX --level $levelX --allPlots --verbose
	# python makeHistograms.py -c $channel -y $year --$controlRegion -s $mysample --syst $systX --level $levelX --plot presel_Njet --plot phosel_Njet --verbose
done
wait
#loop finishes
