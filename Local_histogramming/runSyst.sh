	...
	for controlregion in ${CONTROLREGION[@]}; do
    	for systematics in ${SYSTEMATICS[@]}; do
			for level in ${LEVEL[@]}; do
				echo "Ele" $year $controlregion  $systematics  $level
				./makeHistograms_syst.sh "DiEle" $year $controlregion  $systematics  $level  &
				echo "Mu " $year $controlregion  $systematics  $level
				./makeHistograms_syst.sh "DiMu" $year $controlregion  $systematics  $level  &
            done
			wait
