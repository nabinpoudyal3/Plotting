...
if not os.path.exists("upDownTemplateMisID/"+plotDirectory):
	os.mkdir("upDownTemplateMisID/"+plotDirectory)
	

filename = "misIDEle_syst_Prefit.root"
systematicList = ["BTagSF_b","BTagSF_l","MuEff","EleEff","PhoEff","PU","Q2","isr", "fsr", "prefireEcal","JECTotal", "JER"] # keep adding more systematics 
...
for idir in myDir.keys():
	...
	for item in systematicList:
		...
		ratioUp.GetXaxis().SetTitle('m_{l,#gamma} GeV %s'%item)
		...
		c.Print("upDownTemplateMisID/%s%s_%s.pdf"%(plotDirectory,myDir[idir],item))
		c.Close()
