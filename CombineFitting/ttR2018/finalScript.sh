...
for channel in ${CHANNEL[@]}; do
    ...
    combine -M MultiDimFit datacard_"$channel"_2018.root $mySeed $myAsimovFit --algo grid $myPoints -n .Main_Asimov $mySignalModifierRange  $myParameterRange
    combine -M MultiDimFit datacard_"$channel"_2018.root $mySeed $myAsimovFit --saveWorkspace -n .snapshot_Asimov $mySignalModifierRange  $myParameterRange
    combine -M MultiDimFit higgsCombine.snapshot_Asimov.MultiDimFit.mH120.314159.root $mySeed $myAsimovFit -n .freezeAll_Asimov  --algo grid $myPoints --freezeNuisanceGroups mySyst --snapshotName MultiDimFit $mySignalModifierRange    $myParameterRange
    python plot1DScan.py  higgsCombine.Main_Asimov.MultiDimFit.mH120.314159.root --others 'higgsCombine.freezeAll_Asimov.MultiDimFit.mH120.314159.root:Stat:2' --breakdown Syst,Stat -o freeze_"$channel"_2018_Asimov 

    combine -M MultiDimFit datacard_"$channel"_2018.root $mySeed $myFakeDataFit --algo grid $myPoints -n .Main_Data $mySignalModifierRange    $myParameterRange
    combine -M MultiDimFit datacard_"$channel"_2018.root $mySeed $myFakeDataFit --saveWorkspace -n .snapshot_Data $mySignalModifierRange     $myParameterRange
    combine -M MultiDimFit higgsCombine.snapshot_Data.MultiDimFit.mH120.314159.root $mySeed $myFakeDataFit -n .freezeAll_Data  --algo grid $myPoints --freezeNuisanceGroups mySyst --snapshotName MultiDimFit $mySignalModifierRange    $myParameterRange
