...
Usage: combineCards.py [options] [label=datacard.txt | datacard.txt]

Options:
  -h, --help            show this help message and exit
  -s, --stat            Drop all systematics
  -S, --force-shape     Treat all channels as shape analysis. Useful for mixed
                        combinations
  -a, --asimov          Replace observation with asimov dataset. Works only
                        for counting experiments
  -P FPREFIX, --prefix=FPREFIX
                        Prefix this to all file names
  --xc=CHANNELVETOS, --exclude-channel=CHANNELVETOS
                        Exclude channels that match this regexp; can specify
                        multiple ones
  --ic=CHANNELINCLUDES, --include-channel=CHANNELINCLUDES
                        Only include channels that match this regexp; can
                        specify multiple ones
  --X-no-jmax           FOR DEBUG ONLY: Turn off the consistency check between
                        jmax and number of processes.
  --xn-file=NUISVETOFILE, --exclude-nuisances-from-file=NUISVETOFILE
                        Exclude all the nuisances in this file
  --en-file=EDITNUISFILE, --edit-nuisances-from-file=EDITNUISFILE
                        edit the nuisances in this file

The label=datacard.txt syntax allows to specify the label that channels from
datacard.txt will have in the combined datacard. To combine cards with
different energies one can use dc_7TeV=datacard7.txt dc_8TeV=datacard8.txt
(avoid using labels starting with numbers).
